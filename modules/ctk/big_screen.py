...
def time_2(text1,number,image,x,count = 0,data = None):
    ...
    print(hour)
    print(day)
    img=m_api.image_2(data,day,hour)
    # m_api.image_2(data,day,hour)
...
# def update():
    # time.sleep(10)
    # if m_data.status=="big":
    #     # threading.Thread(target=create).start()
    #     create()
count1 = 0
def create(seconds=0,count2 = None):
    global enter,count1
    if seconds != 0:
        m_data.list_api = []
        m_data.dict_api={"Dnipro":None,"Kyiv":None, "Rome":None, "London":None, "Warsaw":None, "Prague":None}
        m_data.data_api()
    time.sleep(seconds)
    

    if (m_data.status=="big" and count2 == count1)or seconds==0 :
        ...
        text0 = ctk.CTkButton(master=m_data.screen,width=31,height=10,text=m_data.full_name,hover=False,fg_color="#5DA7B1",bg_color="#5DA7B1",command=delete)
        ...
        # delete()
        count1 += 1
        threading.Thread(target=create,daemon=True,args=[1800,count1]).start()
        # update()